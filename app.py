...
HASH_REGISTRY_JSON = "hash_registry.json"  # pre-saved registry file (recommended)
...
# ─────────────────────────────────────────────────────────────────────────────
def detect_tampering(img_bgr: np.ndarray, registry: dict) -> dict:
...
def detect_with_known_id(img_bgr: np.ndarray, image_id: str, registry: dict) -> dict:
    current_hash = DCTWatermark.compute_hash(img_bgr)
...
if mode == "Manual ID entry":
    manual_id = st.text_input(
        ...
        help="Enter the image ID used during watermarking (format: ClassName_stem)",
    )
...
if uploaded is not None:
    ...
    with col2:
        ...
        if result["image_id"]:
            st.markdown(
                f"<div class='section-title'>Image ID</div>"
                f"<div class='hash-box'>{result['image_id']}</div>",
                unsafe_allow_html=True,
            )

        st.markdown(
            f"<div class='section-title' style='margin-top:10px'>Current hash (SHA-256)</div>"
    ...
    # ── Hash diff visualizer (TAMPERED only) ──────────────────────────────────
    ...
    if status == "TAMPERED":
        ...
        with st.expander("Show pixel analysis"):
            ...
            with col_b:
                tamp_file = st.file_uploader(
                    "Suspected tampered image",
                    type=["png", "jpg", "jpeg"],
                    key="tamp_diff",
                )
            ...
            if orig_file and tamp_file:
                orig_bgr = cv2.imdecode(
                    np.frombuffer(orig_file.read(), dtype=np.uint8), cv2.IMREAD_COLOR
                )
                tamp_bgr = cv2.imdecode(
                    np.frombuffer(tamp_file.read(), dtype=np.uint8), cv2.IMREAD_COLOR
                )

                if orig_bgr is not None and tamp_bgr is not None:
                    if orig_bgr.shape != tamp_bgr.shape:
                        tamp_bgr = cv2.resize(
                            tamp_bgr, (orig_bgr.shape[1], orig_bgr.shape[0])
                        )
                    ...
                    c2.image(cv2.cvtColor(tamp_bgr, cv2.COLOR_BGR2RGB),
                             caption="Suspected tampered", use_container_width=True)
